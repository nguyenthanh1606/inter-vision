...
export default function ViewLeads() {
    ...
    return (
      <div className="leads-page">
        ...
        <div className="empty-state">
          ...
          <p>Leads will appear here once visitors interact with your popup widget.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="leads-page">
      <div className="leads-header">
        <div>
          <h2 className="section-title" style={{ marginBottom: 0 }}>📊 Collected Leads</h2>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button className="btn btn-secondary btn-sm" onClick={clearLeads} style={{ color: 'var(--danger)' }}>
            🗑 Clear All
          </button>
        </div>
      </div>
          ...
          <tbody>
            {leads.map((lead) => (
              <tr key={lead.id}>
                ...
                <td>{lead.email || '—'}</td>
                <td>
                  <span className="badge">{new URL(lead.pageUrl).pathname}</span>
                </td>
                <td>
                  <div className="lead-summary" title={lead.summary}>{lead.summary}</div>
                </td>
                <td>
                  <div className="lead-time">{new Date(lead.timestamp).toLocaleString()}</div>
                </td>
              </tr>
            ))}
          </tbody>
