    ...
    <div className="dashboard">
      ...
      <div className="hero-card">
        ...
        <p className="hero-sub">
          AI-powered lead qualification widget. Detect user intent from page content and ask the right questions.
        </p>
      </div>
      ...
      <div className="config-section">
        ...
        <div className="btn-group">
          <button className="btn btn-primary" onClick={handleGenerate} disabled={!config.domain || !config.groqApiKey}>
      ...
      <div className="config-section">
        ...
        <p style={{ color: 'var(--text-2)', fontSize: 14, marginBottom: 16 }}>
          Pre-generate AI questions for each page so the popup loads instantly — no waiting for AI each time.
        </p>

        {/* CLI Instructions */}
        <div style={{ padding: 16, background: 'var(--surface-2)', borderRadius: 'var(--radius-sm)', marginBottom: 20 }}>
          <h4 style={{ marginBottom: 8, color: 'var(--text-1)' }}>📟 CLI (Recommended)</h4>
          <p style={{ color: 'var(--text-2)', fontSize: 13, marginBottom: 10 }}>
            Run the crawl script directly from terminal. Set GROQ_API_KEY first.
          </p>
          <pre style={{ background: '#1a1a2e', color: '#e0e0e0', padding: 14, borderRadius: 8, fontSize: 12, overflowX: 'auto', lineHeight: 1.7 }}>
{`# Set your API key
...
npx tsx apps/smart/scripts/crawl-questions.ts \\
  ...
  --lang vi`}
        ...
        <div style={{ padding: 16, background: 'var(--surface-2)', borderRadius: 'var(--radius-sm)', marginBottom: 16 }}>
          <h4 style={{ marginBottom: 8, color: 'var(--text-1)' }}>🌐 API Crawl (requires dental site running)</h4>
          <p style={{ color: 'var(--text-2)', fontSize: 13, marginBottom: 10 }}>
            If the dental site is running locally, you can crawl via its API endpoint.
          </p>
        </div>
        ...
        {crawling && (
          <div style={{ marginTop: 16, padding: 16, background: 'var(--surface-2)', borderRadius: 'var(--radius-sm)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 20, height: 20, border: '2px solid rgba(255,255,255,0.2)', borderTopColor: 'var(--primary)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
              <span style={{ color: 'var(--text-2)' }}>Đang crawl và tạo câu hỏi AI cho từng trang...</span>
            </div>
          </div>
        )}

        {crawlError && (
          <div style={{ marginTop: 16, padding: 16, background: '#ff4d4f22', border: '1px solid #ff4d4f44', borderRadius: 'var(--radius-sm)', color: '#ff4d4f' }}>
            ❌ {crawlError}
          </div>
        )}
        ...
        {crawlResults && (
          <div style={{ marginTop: 16 }}>
            <h4 style={{ marginBottom: 10, color: 'var(--text-1)' }}>
              ✅ Crawl Complete — {crawlResults.filter((r) => r.status === 'success').length}/{crawlResults.length} pages
            </h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {crawlResults.map((r, i) => (
                ...
                >
                  ...
                  {r.status === 'success' ? (
                    <span style={{ color: '#52c41a' }}>✅ {r.questionsCount} questions</span>
                  ) : (
      ...
      <div className="config-section">
        ...
        <div className="form-grid" style={{ gap: 24 }}>
          <div style={{ padding: 20, background: 'var(--surface-2)', borderRadius: 'var(--radius-sm)' }}>
            <div style={{ fontSize: 32, marginBottom: 8 }}>1️⃣</div>
            <h4 style={{ marginBottom: 6 }}>Crawl & Pre-generate</h4>
            <p style={{ color: 'var(--text-2)', fontSize: 14 }}>
              Crawl your website pages to pre-generate AI questions. The popup will load instantly with cached data.
            </p>
          </div>
          <div style={{ padding: 20, background: 'var(--surface-2)', borderRadius: 'var(--radius-sm)' }}>
            ...
            <p style={{ color: 'var(--text-2)', fontSize: 14 }}>
              If no pre-generated data exists for a page, the widget falls back to real-time AI generation.
            </p>
          </div>
          <div style={{ padding: 20, background: 'var(--surface-2)', borderRadius: 'var(--radius-sm)' }}>
            ...
            <p style={{ color: 'var(--text-2)', fontSize: 14 }}>
              Questions use empathy-driven psychology: ice-breaker → deep need discovery → soft commitment.
            </p>
          </div>
        </div>
      </div>
