  ...
  return (
    <div className={styles.pageLayout}>
      ...
      <div className={styles.contentArea}>
        <div style={{ display: 'flex', gap: 30, marginBottom: 40, background: '#fff', borderRadius: 16, overflow: 'hidden', boxShadow: '0 2px 15px rgba(0,0,0,0.06)' }}>
          <div style={{ flex: 1, background: 'linear-gradient(135deg, #e8f4fd, #b3d9f2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 60, minHeight: 300 }}>📰</div>
          <div style={{ flex: 1, padding: 30, display: 'flex', flexDirection: 'column' as const, justifyContent: 'center' }}>
            <span style={{ display: 'inline-block', background: 'linear-gradient(90deg, #dfaa23, #fbf08d, #dfaa23)', color: '#025899', fontSize: 11, fontWeight: 700, padding: '3px 12px', borderRadius: 10, marginBottom: 12, width: 'fit-content' }}>Implant</span>
            <h3 style={{ fontSize: 22, fontWeight: 700, color: '#025899', marginBottom: 12, lineHeight: 1.4 }}>{tb("p1Title")}</h3>
            <p style={{ fontSize: 14, color: '#666', lineHeight: 1.6, marginBottom: 16 }}>{tb("p1Excerpt")}</p>
            <span className="btn-gold" style={{ width: 'fit-content' }}>{tc("readMore")}</span>
          </div>
        </div>
